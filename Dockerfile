...
RUN mkdir -p /etc/x-ui /var/log/x-ui /etc/xray /app/bin /usr/local/bin && \
    cd /tmp && \
...
# Download Xray core - put EVERYWHERE 3x-ui might look
RUN cd /tmp && \
    wget -q https://github.com/XTLS/Xray-core/releases/latest/download/Xray-linux-64.zip && \
    unzip -o Xray-linux-64.zip && \
    cp xray /usr/local/bin/xray && \
    cp xray /usr/bin/xray && \
    cp xray /app/bin/xray-linux-amd64 && \
    cp xray /bin/xray-linux-amd64 && \
    chmod +x /usr/local/bin/xray /usr/bin/xray /app/bin/xray-linux-amd64 /bin/xray-linux-amd64 && \
